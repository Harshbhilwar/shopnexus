...
const FooterTop = () => {
  return <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 border border-b">
    {data?.map((item,index) => (
      <div key={index} className="flex items-center gap-3 group hover:bg-gray-100 p-4 transition-colors hoverEffect">
        ...
        <div>
          ...
          <p className="text-gray-600 text-sm mt-1 hover:text-gray-900 hoverEffect ">{item?.subtitle}</p>
        </div>
      </div>
    ))}
  </div>;
